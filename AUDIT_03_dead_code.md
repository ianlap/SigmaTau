...
**Category:** ~~Orphaned~~ → **CORRECTION: LIVE** (missed in initial audit)
**Evidence:** `.github/workflows/python-validation.yml:32,35` invokes both scripts on every push/PR to `main`. The audit missed this because `.github/` was in the out-of-scope list for the file-inventory pass — but scripts referenced from CI are live, regardless of where the invocation lives.
**Recommendation:** **Keep.** Do not delete. Consider moving to a dedicated `tests/python/` directory and updating the workflow paths in a separate cleanup pass; placement is awkward but the tests themselves are load-bearing cross-validation (allantools ↔ Stable32 and allantools ↔ SigmaTau).
**Rationale:** Correction to the audit's own finding — the "orphaned, unclear invocation" claim was wrong. `.github/` being out of scope for the audit document itself does not make files it references dead.
