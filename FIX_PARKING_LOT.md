...
- [scripts/julia/kf_pipeline.jl migration] `optimize_nll` return type (`ClockNoiseParams`) drops `.n_evals` and `.converged` from the old `OptimizeResult`. `_nelder_mead` computes both but the wrapper discards them. Scripts now hard-wire `converged=true` as a placeholder and re-evaluate NLL via `innovation_nll` post-hoc.
