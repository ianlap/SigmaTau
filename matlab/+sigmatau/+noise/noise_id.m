...
for k = 1:numel(m_list)
    ...
    try
        ...
        switch err.identifier
            case {'SigmaTau:noise_id:ShortData', 'SigmaTau:noise_id:InvalidDataType'}
                warning(err.identifier, 'Estimation failed for m=%d: %s', m, err.message);
            otherwise
                rethrow(err);
        end
    end
...
elseif strcmpi(data_type, 'freq')
    ...
    error('SigmaTau:noise_id:InvalidDataType', 'data_type must be ''phase'' or ''freq''');
end
...
while true
    ...
    d = d + 1;
    if numel(x) < 5
        error('SigmaTau:noise_id:ShortData', 'Data too short after differencing');
    end
end
end
